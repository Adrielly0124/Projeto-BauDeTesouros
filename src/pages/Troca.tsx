...
export default function Troca() {
  ...
  useEffect(() => {
    async function carregar() {
      try {
        const itens = await listarItensPorTipo("troca");

        // 🔥 Agora usando os campos certos do Firestore
        const mapped: Produto[] = itens.map((it: any) => ({
          id: it.id,
          name: it.titulo,                 // ✔ título
          kind: it.tipo,                   // ✔ tipo
          imagem: it.imagens?.[0] || ""    // ✔ primeira imagem
        }));

        setData(mapped);

      } catch (err) {
        console.error("Erro ao carregar itens de troca:", err);
      ...
      <section className="bt-list" style={{ marginTop: 18 }}>
        ...
        {!loading && data.length === 0 && (
          <p style={{ textAlign: "center", opacity: 0.7 }}>
            Nenhum item encontrado para troca.
          </p>
        )}
